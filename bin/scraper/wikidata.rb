...
query = <<SPARQL
  ...
  WHERE {
    ...
    OPTIONAL { ?item rdfs:label ?name FILTER(LANG(?name) = "fi") }

    OPTIONAL { ?positionItem wdt:P1705 ?nativeLabel }
    OPTIONAL { ?positionItem rdfs:label ?positionLabel FILTER(LANG(?positionLabel) = "fi") }
    BIND(COALESCE(?nativeLabel, ?positionLabel) AS ?position)
  }
  ORDER BY ?positionLabel ?began
SPARQL

agent = 'every-politican-scrapers/finland-cabinet'
